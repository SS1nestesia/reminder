...
func (h *FriendHandlers) handleTextTimeForFriend(ctx *th.Context, chatID int64, sessionID int, friendID int64, input string) error {
	...
	userLoc := h.service.GetUserLocation(ctx.Context(), chatID)
	name := h.common.ResolveName(ctx.Context(), friendID)
	confirmation := fmt.Sprintf("✅ <b>Напоминание для %s создано!</b>\n⏰ %s",
		name, t.In(userLoc).Format("02.01.2006 15:04"))

	// Notify the friend
	friendNotification := fmt.Sprintf("🔔 <b>Друг создал для вас напоминание!</b>\n\n📝 %s\n⏰ %s",
		html.EscapeString(pendingText), t.In(userLoc).Format("02.01.2006 15:04"))
	_ = h.common.send(ctx.Context(), friendID, friendNotification, nil)

	// Store the reminder ID if we need recurrence later
	_ = h.state.SetPendingReminder(ctx.Context(), chatID, newID)

	tz, _ := h.state.GetTimezone(ctx.Context(), chatID)
	pendingCount := h.common.getPendingCount(ctx.Context(), chatID)
	return h.common.edit(ctx.Context(), chatID, sessionID, confirmation, MainMenuKeyboard(tz, pendingCount).(*telego.InlineKeyboardMarkup))
}
